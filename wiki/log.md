...
- Verification: `uv run pytest -q -m 'not integration'` -> `181 passed, 44 deselected`; registry-backed startup RED tests now pass, and `uv run python` previously confirmed live `docmesh_py_core` import and registry creation; current lock/runtime package version remains `0.1.1` despite `pyproject.toml` declaring `>=0.1.4`
- Updated: `index.md`
