...
- Verification: `uv run pytest -q -m 'not integration'` -> `185 passed, 44 deselected`; registry-first fallback RED tests now pass and the broader dependency/lifecycle subset passes `62` tests; current lock/runtime package version remains `0.1.1` despite `pyproject.toml` declaring `>=0.1.4`
- Updated: `index.md`
