...
문서상 대표 통합 대상은 PostgreSQL, SQLite, MinIO, NATS, Keycloak이다. 이들 통합은 [[service-factory-registry]] 를 중심으로 조립되며, 저장소 선택은 [[environment-driven-service-selection]] 패턴을 따른다.

FastAPI lifespan, 배치/CLI 진입점, startup readiness 검증은 [[sdk-health-check-patterns]] 와 밀접하게 연결된다. 인증 관련 사용 사례는 [[keycloak-auth-integration]] 에 별도로 정리한다.
