...
sources: [raw/articles/fastapi-core-prd-2026-06-16.md, raw/articles/fastapi-core-api-2026-06-17.md, raw/articles/fastapi-core-config-2026-06-17.md, raw/articles/fastapi-core-messaging-2026-06-18.md]
confidence: medium
---
# fastapi-core

## Overview
`fastapi-core` 는 DocMesh 계열 FastAPI 서비스에서 공통으로 사용하는 Python SDK 패키지다. 2026-06-18 기준 문서는 이 패키지를 인증, 데이터베이스, 스토리지, 벡터 DB, 로컬 LLM, 관측성, 메시징, 그리고 FastAPI 앱 조립을 함께 표준화하는 공통 인프라 레이어로 정의한다.
...
메시징 가이드는 이 제품 설명을 NATS 도메인까지 확장한다. `fastapi_core.core.messaging` 의 subject/publish/subscribe helper 와 `fastapi_core.dependencies.messaging` 의 registry-backed FastAPI dependency 경로를 분리해 설명하면서, standalone helper API와 앱 수명주기 통합 API가 같은 SDK 안에서 어떻게 공존하는지 더 구체적으로 보여준다.
...
- NATS subject 검증/조합, JSON publish/subscribe helper, queue subscription 패턴 제공
...
메시징 영역에서는 [[nats-event-helper-layer]] 가 보여주듯 core helper 와 dependency helper 의 경계를 의도적으로 분리한다. 즉 raw NATS client 생성 및 subject/payload 규칙은 helper 레이어에서, 기본 FastAPI 객체 해석과 state 캐시는 registry-backed dependency 레이어에서 담당한다.
...
- [[nats-event-helper-layer]] 는 메시징 영역에서 helper API와 dependency API가 어떻게 분리되는지 설명한다.
