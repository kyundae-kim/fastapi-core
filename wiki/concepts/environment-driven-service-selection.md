...
## Operational Notes
SQLite는 `SQLITE_PATH=:memory:` 같은 설정으로 테스트 환경에서 매우 가볍게 사용할 수 있으며, `SQLITE_ENABLE_WAL`, `SQLITE_BUSY_TIMEOUT_MS` 로 동작을 조정할 수 있다. PostgreSQL은 `POSTGRES_DSN` 이 있으면 개별 host/user/password 보다 우선한다.
