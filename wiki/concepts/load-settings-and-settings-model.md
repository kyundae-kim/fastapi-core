...
## Validation Contract
`load_settings()` 는 단순 파싱이 아니라 필수값 누락, 형식 오류, 상호배타 규칙 위반, 운영 보안 규칙 위반을 검증하고 실패 시 `ConfigError` 를 발생시킨다. 따라서 [[environment-driven-service-selection]] 패턴이 환경에 의존하더라도, 잘못된 구성은 초기화 초기에 드러낼 수 있다.
