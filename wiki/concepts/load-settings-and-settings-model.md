...
sources: [raw/articles/docmesh-py-core-sdk-2026-06-11.md, raw/articles/docmesh-py-core-api-2026-06-11.md, raw/articles/docmesh-py-core-config-2026-06-11.md, raw/articles/fastapi-core-config-2026-06-17.md]
confidence: medium
---
...
## What It Produces
`docmesh-py-core` 의 `Settings` 는 패키지의 최상위 설정 컨테이너이며 `common`, `keycloak`, `postgres`, `sqlite`, `minio`, `milvus`, `ollama`, `langfuse`, `nats` 같은 서비스별 설정을 묶어 제공한다. 반면 `fastapi-core` 는 이와 유사한 역할을 `EnvConfig` + `ServiceSettings` 의 이중 레이어로 분리한다. 이후 애플리케이션은 이 객체들을 [[service-factory-registry]] 또는 FastAPI lifecycle/dependency 계층에 전달해 런타임 통합을 구성한다.
...
- [[layered-configuration-model]] 은 `fastapi-core` 가 왜 `EnvConfig` 와 `ServiceSettings` 로 책임을 나누는지 설명한다.
