...
## Definition
`docmesh-py-core` 와 `fastapi-core` 의 설정 모델은 외부 서비스 연결 정보를 환경변수에서 읽고, 애플리케이션 시작 시 1회 로드·검증하는 원칙에 기반한다. `fastapi-core` 는 여기에 YAML 기반 서비스 정책 레이어를 추가하지만, URL, 계정, 비밀번호, token, secret key를 코드에 하드코딩하지 않는다는 기본 전제는 동일하다.

## Core Rules
공백 문자열은 값이 없는 것으로 간주하고, boolean 값은 대소문자와 관계없이 `true` / `false` 로 해석한다. 숫자형 값은 허용 범위를 검증하며, 공통 timeout/retry/pool 설정을 두지 않고 서비스별 환경변수로 분리한다. `fastapi-core` 설정 문서는 알 수 없는 환경변수를 `extra="ignore"` 로 무시하고, `env_nested_delimiter="__"` 기반의 중첩 환경변수 모델을 공식 계약으로 둔다. 이 원칙은 [[load-settings-and-settings-model]] 및 [[layered-configuration-model]] 의 검증 계약을 구체화한다.
...
- [[layered-configuration-model]] 은 환경변수 레이어와 YAML 레이어의 역할 분리를 설명한다.
