...
권장 흐름은 [[load-settings-and-settings-model]] 에 정리된 `load_settings()`로 검증된 설정을 만든 뒤 `ServiceFactoryRegistry(settings)` 를 생성하고, 필요한 서비스에 대해 `create_client("postgres")`, `create_client("minio")` 같은 호출을 수행하는 것이다. 실제 연결 검증은 [[sdk-health-check-patterns]] 에 따라 startup 시점에 `check()` 또는 서비스별 확인 루틴으로 수행한다.
...
## Notable Caveat
API 문서 기준 registry의 지원 서비스명은 `keycloak`, `postgres`, `sqlite`, `minio`, `milvus`, `ollama`, `langfuse`, `nats` 이다. 이 중 대부분은 [[service-client-wrapper]] 를 반환하지만, Langfuse는 비활성화 시 `None` 일 수 있고 NATS는 일반 동기 client가 아니라 [[nats-connection-builder]] 를 반환한다. 따라서 호출자는 서비스별 반환 계약 차이를 이해해야 한다.

## Relationships
- [[docmesh-py-core]] 의 핵심 통합 패턴을 구현한다.
- [[load-settings-and-settings-model]] 에서 생성된 설정 객체를 입력으로 받는다.
- [[environment-driven-service-selection]] 에 의해 어떤 서비스를 생성할지 결정된다.
- 대부분의 서비스는 [[service-client-wrapper]] 로 노출된다.
- `nats` 는 [[nats-connection-builder]] 로 분기되는 예외다.
- [[sdk-health-check-patterns]] 및 [[check-all-services]] 와 함께 startup readiness 및 종료 정리를 구성한다.
