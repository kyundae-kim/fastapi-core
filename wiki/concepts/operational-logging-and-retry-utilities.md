...
`docmesh-py-core`는 서비스 연결 코드 주변의 운영 보일러플레이트를 줄이기 위해 `mask_sensitive_value`, `configure_logging`, `build_service_log_event`, `retry_call`, `close_service_clients`를 패키지 루트 공개 helper 표면으로 제공한다.^[raw/articles/docmesh-py-core-api-reference-v0.5.0.md]
